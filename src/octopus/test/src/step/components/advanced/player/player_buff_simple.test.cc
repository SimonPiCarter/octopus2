...
struct ProdPlayerBuffTestSimple : ProductionTemplate<StepManager<DEFAULT_STEPS_T>>
{
    ...
    virtual void produce(flecs::entity producer_p, flecs::world const &ecs, StepManager<DEFAULT_STEPS_T> &manager_p) const
	{
		...
		if(player.is_valid())
		{
			AddComponentStep< PlayerBuff<Ranger, PlayerBuffTestSimple, HitPoint> > step {{PlayerBuffTestSimple {2}}};
...
struct ProdUnitTestSimple : ProductionTemplate<StepManager<DEFAULT_STEPS_T>>
{
    ...
    virtual void produce(flecs::entity producer_p, flecs::world const &ecs, StepManager<DEFAULT_STEPS_T> &manager_p) const
	{
		...
		step_l.set_up_function = [](flecs::entity e, flecs::world const &) {
...
TEST(player_buff_simple, simple)
{
	...
	ecs.component<PlayerBuffTestSimple>()
	...
	declare_player_buff_systems<Ranger, PlayerBuffTestSimple, HitPoint>(ecs);
