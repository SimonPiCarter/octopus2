...
// custom component to enable damage
struct Aggressive {
	Fixed dmg;
};
...
TEST(custom_idle_loop, simple)
{
	...
	auto e1 = ecs.entity()
		...
		.set<Aggressive>({Fixed(2)});

	auto e2 = ecs.entity()
		.set<HitPoint>({Fixed(10)});

	ecs.system<Aggressive const>()
		.kind(flecs::OnValidate)
		.each([&res, &containers_l](flecs::entity& e, Aggressive const &aggro_p) {
			containers_l.back().hitpoints.add_step(e, {-aggro_p.dmg});
			res<<" a"<<hp_p.qty.to_int();
		});
