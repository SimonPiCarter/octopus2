...
/// Index into Triangulation::_points (size_t(-1) for super-triangle vertices)
using PointIdx = std::size_t;

static constexpr PointIdx SUPER_A = std::size_t(-1);
static constexpr PointIdx SUPER_B = std::size_t(-2);
static constexpr PointIdx SUPER_C = std::size_t(-3);
...
public:
    ...
    /// Access user-visible triangles (those not touching super-triangle vertices).
    std::vector<Triangle> const &triangles() const;

    /// Access point by index.
    TriPoint const &point(PointIdx idx) const;

    /// Number of user points (excluding super-triangle).
    std::size_t pointCount() const { return _points.size(); }

private:
    std::vector<TriPoint> _points; ///< user points indexed by PointIdx
    TriPoint _superA, _superB, _superC; ///< super-triangle vertices

    std::vector<Triangle> _triangles; ///< all triangles including super-triangle ones
    ...
    /// Bowyer-Watson insertion step (works with raw index, including super vertices)
